...
foreach (var antenna in antennas)
{
    ...
    foreach (var antenna2 in fellowAntennas)
    {
        ...
        //the antennas inseft:
        antinodes.Add(antenna.Location);
        antinodes.Add(antenna2.Location);
        
        //calc vektor
        var vek  = antenna.Location - antenna2.Location;

        //all antinotes in the one direction
        int multiplier = 1;
        Point2D tempantinode;
        tempantinode = antenna.Location + vek * multiplier++;
        while (!IsOutOfBounds(tempantinode))
        {
            antinodes.Add(tempantinode);
            tempantinode = antenna.Location + vek * multiplier++;
        }
        
        //and the antinotes the the other direction
        multiplier = 1;
        tempantinode = antenna2.Location - vek * multiplier++;
        while (!IsOutOfBounds(tempantinode))
        {
            antinodes.Add(tempantinode);
            tempantinode = antenna2.Location - vek * multiplier++;
